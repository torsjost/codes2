...
* Compute orbital magnetization LC contribution!!
* Read in IC contribution from haldane3.f !!

      implicit real*8 (a-h,o-z)
      parameter (maxp=1500,kdim=2,maxk=160000)
     ...
     .                 e1(2)
      ...
      double precision MICtemp

      double precision mingap,tempgap

      double precision kdumx,kdumy

      double precision numk
      double precision tempkx,tempky 
      double precision h1xp(2,2,2),h1xm(2,2,2)
      double precision h1yp(2,2,2),h1ym(2,2,2)
      double precision e1xp(2)  !kx+d
      double precision e1xm(2)  !kx-d
      double precision e1yp(2)  !ky+d
      double precision e1ym(2)  !ky-d
      double precision uprim,uhubb
      double precision qx,qy

      complex*16 Gnnp(2,2,maxk)  ! Gnnp is new 
      complex*16 dGnnpdx(2,2,maxk) ! new
      complex*16 dGnnpdy(2,2,maxk) ! new

      double precision re11,re12,re21,re22
      double precision im11,im12,im21,im22
      ...
      double precision phiC(maxp), ekn(2) 
      double precision MIC(maxp), tx(2), ty(2)
      complex*16 uknbig(maxk,2,2)
      complex*16 eknbig(maxk,2)

      complex*16 b(2,2,8), xi, csum, csum1, csum2, csum3
      complex*16 ctem1, ctem2 
      ...
      complex*16 vx(2,2),vy(2,2)
      complex*16 vxukn(2,2),vyukn(2,2)
      complex*16 volapx(2,2),volapy(2,2)
      ...
      complex*16 Hint11old,Hint12old,Hint21old,Hint22old
      complex*16 Hint12_1old,Hint12_2old,Hint12_3old
...
* Open files.
      ...
      open(unit=2,file='Morb4-gw.d',status='UNKNOWN')  !LC
      open(unit=22,file='Morb3-k.d',status='UNKNOWN')
      open(unit=23,file='Mtot-k.d',status='UNKNOWN')
      open(unit=3,file='M-tot-gw.d',status='UNKNOWN')
      open(unit=4,file='Morb3-gw.d',status='UNKNOWN')  !IC

      open(unit=117,file='ReG.d',status='UNKNOWN') 
      open(unit=118,file='ImG.d',status='UNKNOWN') 
      open(unit=119,file='RedGdx.d',status='UNKNOWN') 
      open(unit=120,file='ImdGdx.d',status='UNKNOWN') 
      open(unit=121,file='RedGdy.d',status='UNKNOWN') 
      open(unit=122,file='ImdGdy.d',status='UNKNOWN') 


      ! ALSO HAVE TO READ FROM THE dG/dq STUFF
...
* Basis!
      ...
      ty(1) = 0.d0
      ty(2) = 1.d0
      write(6,*)'With phasefacors!!'
      else
      tx(1) = 0.d0
      tx(2) = 0.d0
      ty(1) = 0.d0
      ty(2) = 0.d0
      write(6,*)'No phasefacors!!'
      endif


! Contrary to in the IC routine, we don't include the boundrary here
! (maybe because of the Chern number, which is calculated in IC?)

* Calculate M.
      volwgt = ( (8.d0*pi*pi)/(3.d0*dsqrt(3.d0)) )*
     .         ( 1.d0/((nkabc(1))*(nkabc(2))) )
!      volwgt = ( (8.d0*pi*pi)/(3.d0*dsqrt(3.d0)) )*    !No boundary
!     .         ( 1.d0/((nkabc(1))*(nkabc(2))) )
      numk = dble(nkabc(1)*nkabc(2))
...
* Choose yy = Delta/t2 !!
      yy = 6.0d0    !Thonhauser PRL 2005 E0=2 and t2=1/3
*      yy = 5.20d0    !Thonhauser PRL 2005 E0=2 and t2=1/3
*      yy = 3.67d0
*      yy = 3.00d0     !Ceresoli PRB74, 024408  (eA = 1)
*      yy = 1.00d0
*      yy = 0.10d0
...
* Without boundary we have (2*nkabc(1)+1) fewer points compared with the boundary case!!
      ...
      do ix = 1, nkabc(1)   !Also boundary
*      do ix = 1, nkabc(1)      !No boundary 2->3 and 3->4 

      ux=ux+dux
      uy=-duy

      do iy = 1, nkabc(2)     !Also boundary 
...
* phi-mesh!
      ...
      write(6,*)'Read LC contribution from file!'
      rewind(4)
!      read(4,*)

      do iUUp = 0,50! nphi
      read(4,'(i4,1f26.18)') funint,MIC(iUUp)
      write(6,'(i4,1f26.18)') funint,MIC(iUUp)
      enddo
...
!      if (iUUp.gt.10)then
!      uhubb1=6.d0
!      uprim=(dble(iUUp)-10.d0)/10.d0*6.d0
!      endif 
...
!      ffk(icc,ib)    = 1.d0/(1.d0+dexp( beta*(e1(ib)-rmu0) ))
      do i1      = 1,kdim
      uknbig(icc,i1,ib) = dcmplx(z1(i1,ib,1),z1(i1,ib,2))
      enddo
      enddo
...
!      print *,'kalle'
      ...
      Hint11=Hint11 + dconjg(uknbig(icc,2,1))*uknbig(icc,2,1)
      Hint12_1=Hint12_1 + dconjg(uknbig(icc,2,1))*uknbig(icc,1,1)*
     .        cdexp(xi*qy)
      Hint12_2=Hint12_2 + dconjg(uknbig(icc,2,1))*uknbig(icc,1,1)*
     .        cdexp(-xi*( dsqrt(3d0)/2d0*qx + (1d0/2d0)*qy ) )
      Hint12_3=Hint12_3 + dconjg(uknbig(icc,2,1))*uknbig(icc,1,1)*
     .        cdexp(xi*( dsqrt(3d0)/2d0*qx - (1d0/2d0)*qy ) )
      Hint22=Hint22 + dconjg(uknbig(icc,1,1))*uknbig(icc,1,1)

      Hon11=Hon11 + dconjg(uknbig(icc,1,1))*uknbig(icc,1,1)
      Hon22=Hon22 + dconjg(uknbig(icc,2,1))*uknbig(icc,2,1)  
...
!      stop
      ...
      Hon11=Hon11*uhubb1/numk
      Hon22=Hon22*uhubb1/numk
      endif
...
!     .      alpha2*Hon22old
     ...
     .          Hon22old*dconjg(Hon22old) ) ) )
      ...
      read (117,'(i4,5f12.6)') ip0, phi0         
      read (118,'(i4,5f12.6)') ip0, phi0           
      read (119,'(i4,5f12.6)') ip0, phi0          
      read (120,'(i4,5f12.6)') ip0, phi0         
      read (121,'(i4,5f12.6)') ip0, phi0           
      read (122,'(i4,5f12.6)') ip0, phi0          
...
!      print *,'apa'
!!!!! G
      read (117,'(i4,6f26.18)') iq, tempkx,tempky,
     .                          re11,re12,re21,re22
      read (118,'(i4,6f26.18)') iq, tempkx,tempky,
     .                          im11,im12,im21,im22

      Gnnp(1,1,icount)=dcmplx(re11,im11)
      Gnnp(1,2,icount)=dcmplx(re12,im12)
      Gnnp(2,1,icount)=dcmplx(re21,im21)
      Gnnp(2,2,icount)=dcmplx(re22,im22)

      read (119,'(i4,6f26.18)') iq, tempkx,tempky,
     .                          re11,re12,re21,re22
      read (120,'(i4,6f26.18)') iq, tempkx,tempky,
     .                          im11,im12,im21,im22

      dGnnpdx(1,1,icount)=dcmplx(re11,im11)
      dGnnpdx(1,2,icount)=dcmplx(re12,im12)
      dGnnpdx(2,1,icount)=dcmplx(re21,im21)
      dGnnpdx(2,2,icount)=dcmplx(re22,im22)

      read (121,'(i4,6f26.18)') iq, tempkx,tempky,
     .                          re11,re12,re21,re22
      read (122,'(i4,6f26.18)') iq, tempkx,tempky,
     .                          im11,im12,im21,im22

      dGnnpdy(1,1,icount)=dcmplx(re11,im11)
      dGnnpdy(1,2,icount)=dcmplx(re12,im12)
      dGnnpdy(2,1,icount)=dcmplx(re21,im21)
      dGnnpdy(2,2,icount)=dcmplx(re22,im22)
...
* For later use!
      do ib      = 1,kdim  !Band
      ekn(ib) = e1(ib) - rmu
*      ekn(ib) = e1(ib)    !If 2*rmu in haldane3.f
      do i1      = 1,kdim
...
* H21.
      ...
      do ib2=1,kdim
      do ib3=1,kdim
      h1xp(ib3,ib2,ib1)=h1(ib3,ib2,ib1)
      enddo
      enddo
      enddo
...
! MODIFY LIKE I DID BEFORE
* Determine dual state! Band 1 only occupied. See Ceresoli PRB74, Eq. A11-12.
      do ib    = 1,kdim 
      csum=dcmplx(0d0,0d0)
      do i1 = 1,kdim
      csum  = csum+dcmplx(z1  (i1,ib,1),-z1  (i1,ib,2))* ! MODIFY!!!!!!
...
* H21.
      ...
      do ib2=1,kdim
      do ib3=1,kdim
      h1xm(ib3,ib2,ib1)=h1(ib3,ib2,ib1)
      enddo
      enddo
      enddo
...
* H21.
      ...
      do ib2=1,kdim
      do ib3=1,kdim
      h1yp(ib3,ib2,ib1)=h1(ib3,ib2,ib1)
      enddo
      enddo
      enddo
...
* H21.
      ...
      do ib2=1,kdim
      do ib3=1,kdim
      h1ym(ib3,ib2,ib1)=h1(ib3,ib2,ib1)
      enddo
      enddo
      enddo
...
* For kx,ky-dy!!
     ...
     .               cdexp(xi*dx*( tx(i1)))
      ...
      do i1=1,kdim
      do i2=1,kdim
      vx(i1,i2)=(dcmplx(h1xp(i1,i2,1),h1xp(i1,i2,2))-
     .           dcmplx(h1xm(i1,i2,1),h1xm(i1,i2,2)))/
     .           (2d0*dx)
      vy(i1,i2)=(dcmplx(h1yp(i1,i2,1),h1yp(i1,i2,2))-
     .           dcmplx(h1ym(i1,i2,1),h1ym(i1,i2,2)))/
     .           (2d0*dy)
...
!      endif
      ...
      do i1=1,kdim ! no sum
      do ib1=1,kdim ! no sum
      vxukn(i1,ib1)=dcmplx(0d0,0d0)
      vyukn(i1,ib1)=dcmplx(0d0,0d0)
      do i2=1,kdim ! SUM
      vxukn(i1,ib1)=vxukn(i1,ib1)+vx(i1,i2)*ukn(i2,ib1)
      vyukn(i1,ib1)=vyukn(i1,ib1)+vy(i1,i2)*ukn(i2,ib1)
      enddo
      ...
      do ib1=1,kdim
      do ib2=1,kdim
      volapx(ib1,ib2)=dcmplx(0d0,0d0)
      volapy(ib1,ib2)=dcmplx(0d0,0d0)
      do i1=1,kdim
      volapx(ib1,ib2)=volapx(ib1,ib2)+dconjg(ukn(i1,ib1))*vxukn(i1,ib2)
      volapy(ib1,ib2)=volapy(ib1,ib2)+dconjg(ukn(i1,ib1))*vyukn(i1,ib2)
      enddo
      enddo
      enddo
      ...
      do i1=1,kdim
      do ib2=1,kdim ! We do NOT sum over the right band index (n)

      derxuk(i1,ib2)=dcmplx(0d0,0d0)
      deryuk(i1,ib2)=dcmplx(0d0,0d0)

      do ib1=1,kdim ! We sum over the left band index (m)

      if(ib1.ne.ib2)then ! Perturbation (xi=sqrt(-1))
      derxuk(i1,ib2)=derxuk(i1,ib2)+
     .               ukn(i1,ib1)*volapx(ib1,ib2)/(ekn(ib2)-ekn(ib1))
      deryuk(i1,ib2)=deryuk(i1,ib2)+
     .               ukn(i1,ib1)*volapy(ib1,ib2)/(ekn(ib2)-ekn(ib1))
      ...
      do i1=1,kdim
      do ib1=1,kdim
      ukxp(i1,ib1)=ukn(i1,ib1)+derxuk(i1,ib1)*dx
      ukxm(i1,ib1)=ukn(i1,ib1)-derxuk(i1,ib1)*dx
      ukyp(i1,ib1)=ukn(i1,ib1)+derxuk(i1,ib1)*dy
      ukym(i1,ib1)=ukn(i1,ib1)-derxuk(i1,ib1)*dy
      enddo
      enddo
...
*** New!!
* Dual state orthogonal to ukn if bandindex different!!
* This means that ib=1 only in sum below i.e ctem1=0.

! WE NO LONGER DETERMINE THE DERIVATIVES       

! Note that we have put in the completeness sum_{n'} |u_n'><u_{n'|=1
* Term1:
      csum11 = dcmplx(0.d0,0.d0)

      do ib1=1,kdim ! / Tor
      do ib2=1,kdim ! / Tor
      do ib  = 1,kdim  !Band  index t

      ctem1  = dcmplx(0.d0,0.d0)
      ctem2  = dcmplx(0.d0,0.d0)

      do i1  = 1,kdim   !Sum over basis 
      ctem1  = ctem1 + dconjg(derxuk(i1,ib1))*ukn(i1,ib)
      ctem2  = ctem2 + dconjg(ukn(i1,ib))*deryuk(i1,ib2)*
     .                 cdexp( xi*dy*ty(i1))
      enddo

      ! This thing is added for each band m
      ! PLUS
      csum11 = csum11 + ctem1*ctem2*ekn(ib)*Gnnp(ib2,ib1,icount) 
...
* Term2:
      ...
      do i1  = 1,kdim   !Sum over basis 
      ctem1  = ctem1 + dconjg(deryuk(i1,ib1))*ukn(i1,ib)
      ctem2  = ctem2 + dconjg(ukn(i1,ib))*derxuk(i1,ib2)*
     .                 cdexp( xi*dy*ty(i1))
      enddo

      ! This thing is added for each band m
      ! PLUS
      csum12 = csum12 - ctem1*ctem2*ekn(ib)*Gnnp(ib2,ib1,icount) 

      enddo
      enddo
      enddo

      csum13 = dcmplx(0.d0,0.d0)
      ...
      do i1=1,kdim
      ctem1 = ctem1 + dGnnpdy(ib2,ib1,icount)*
     .                dconjg(derxuk(i1,ib1))*ukn(i1,ib2)-
     .                dGnnpdx(ib2,ib1,icount)*
     .                dconjg(deryuk(i1,ib1))*ukn(i1,ib2)
      enddo 
  
      csum13=csum13+ctem1*ekn(ib2)
...
!      print *,'heeeej'
      ...
      csum1 = csum1 + csum11 + csum12 + csum13 ! Adding term 1-8

      cdumk=csum1+csum11+csum12+csum13

!      if(iUUp.eq.14)then
!      read(22,'(3f26.18)') kdumx,kdumy,MICtemp
!      write(23,'(3f26.18)') kdumx,kdumy,
!     .                      MICtemp-dimag(cdumk*volwgt*fact)*2.d0
!      endif
      ...
      if(iUUp.eq.0) write(6,'(a30,3f10.2)')'LC contribution to M:'
      if(iUUp.eq.0) write(2,'(a30,3f10.2)')'LC contribution to M:'
!      write(6,'(a30,i4,1f26.18)')'M:',iUUp,-dimag(csum1*volwgt*fact)*2.d0
      !FA factor 2
      write(2,'(i4,1f26.18)'),iUUp,-dimag( csum1*volwgt*fact )*2.d0   
      !FA factor 2


      write(3,'(i4,1f26.18)'),iUUp,
*     .( -dimag( csum1*volwgt*fact )*2.d0 + MIC(ip) ) 
     .    ( -dimag( csum1*volwgt*fact )*2.d0 + MIC(iUUp) )
!     .   *( 1.5d0*dsqrt(3.d0) )  !Mult. with the unit cell area
