...
* Run the DOS calculation first with many k-points!!
      ...
      complex*16 deter0,deter

      complex*16 deter0vals(nq0,-nwm:nwm)

      integer nbr

      double precision deter0abs
      double precision deter0absmean
* Old !
*      parameter (ntaum= 2*52+1)
      parameter (ntaum= 4*20+1) ! 1000   !OK for nsimp=2 and 4
*      parameter (ntaum= 106)
...
*      parameter (nsimp= 2)
      ...
      double precision xx
      double precision diff,diffold
      double precision wi
      double precision eA,eB,t1,t2,phi
      double precision rtemp1,rtemp2,rtemp3 
      double precision mumax

      complex*16 trG(0:nwm-1),trGold(0:nwm-1),trGwrite(0:nwm-1)
      complex*16 rsum3
      double precision analytic(ntaum)
      complex*16 analytic_pos(ntaum)
      complex*16 analytic_neg(ntaum)
...
* For W !!
      double precision rwe(-nwm:nwm),cwe(-nwm:nwm),
     .                 wcoswt(-nwm:nwm,ntaum),
     .                 wsinwt(-nwm:nwm,ntaum)  ! / Tor

      complex*16 we(-nwm:nwm)
      complex*16 wexpwt(-nwm:nwm,ntaum)
      complex*16 U12,U21
     
      complex*16 Hint11,Hint12,Hint21,Hint22
      complex*16 Hint12_1,Hint12_2,Hint12_3

      complex*16 Hint11old,Hint22old,Hint12_1old,Hint12_2old,Hint12_3old
      complex*16 Hon11old,Hon22old

      complex*16 Hon11,Hon22
...
* Odd Matsubara !!
      ...
      double precision kx,ky,kx0,ky0,dkx,dky,kyold
...
* Even Matsubara use (-nwm:nwm) !!
      ...
      complex*16 W11opp,W12opp,W21opp,W22opp

* Odd Matsubara use (-nwm:nwm-1) !!
      complex*16 ScTau(2,2,nq0,ntaum)

      complex*16 SigmaC(2,2,nq0,-nwm:nwm-1)
      complex*16 SigmaEx(2,2,nq0)
      
      complex*16 HXC(2,2),UM(2,2),UinvM(2,2)
      complex*16 tempmat(2,2),Smat(2,2)
      complex*16 HXCAtom(2,2,nq0)
      double precision VHartree(2,2,nq0)
      ...
      complex*16 ukn(nq0,2,2), xi, csum, csum1, csum2, csum3
      complex*16 csum4, csum5, csum6, csum7
...
* Open files.
      ...
      open(unit=101,file='P11plot.d',status='UNKNOWN')
      open(unit=102,file='P12plot.d',status='UNKNOWN')
      open(unit=103,file='P21plot.d',status='UNKNOWN')
      open(unit=104,file='P22plot.d',status='UNKNOWN')

      open(unit=105,file='W11plot.d',status='UNKNOWN')
      open(unit=106,file='W12plot.d',status='UNKNOWN')
      open(unit=107,file='W21plot.d',status='UNKNOWN')
      open(unit=108,file='W22plot.d',status='UNKNOWN')

      open(unit=205,file='Niter.d',status='UNKNOWN')
      open(unit=206,file='Hint.d',status='UNKNOWN')
      open(unit=207,file='Hintsum.d',status='UNKNOWN')
      open(unit=208,file='stagg.d',status='UNKNOWN')


      open(unit=110,file='deter.d',status='UNKNOWN')
...
!      open(unit=52,file='Gtau22.d',status='UNKNOWN')
      ...
      open(unit=77,file='eigen.d',status='UNKNOWN')

      open(unit=192,file='rHXC.d',status='UNKNOWN')
      open(unit=193,file='iHXC.d',status='UNKNOWN')
...
* Define !!
      ...
      if(MOD(nw,2).ne.0) stop 'not even'
...
* Test numerically G(iw) to G(tau) !!
      ...
      eu  = -0.5d0    ! E-mu
      ffu = 1.d0/(1.d0+dexp(beta*eu)) ! Fermi factor
...
* Matsubara mesh, Even for P0 and W, Odd for Sigma 
      dw = 2.d0*pi/beta
...
* Odd Matsubara !!
      ...
      do it=1,ntau
      do iv=-nw,nw
      wexpwt(iv,it)=exp(-xi*wkP(iv)*tau(it))/beta
      enddo
      enddo

      do it=1,ntau
      analytic_pos(it)=dcmplx(0.d0,0.d0)
      analytic_neg(it)=dcmplx(0.d0,0.d0)
      do iv=nw+1,nw+100000
      wi=dw*dfloat(iv)
      analytic_pos(it)=analytic_pos(it)+
     .                 exp(-xi*wi*tau(it))/wi**2.d0
      enddo
      do iv=-nw-100000,-nw-1
      wi=dw*dfloat(iv)
      analytic_neg(it)=analytic_neg(it)+
     .                 exp(-xi*wi*tau(it))/wi**2.d0
      enddo
      enddo
...
* MAIN CODE
      do ip = 1,1!nphi
      do iUUp = 0,50!56,93!NUUp
 
!      print *,'rrr' 
      if (iUUp.le.40)then
      uhubb1=dble(iUUp)/40.0d0*2.0d0
      uprim=0.d0
      endif
      if (iUUp.gt.40)then
      uhubb1=2.d0
      uprim=(dble(iUUp)-40.d0)/40.d0*2.0d0
      endif 
      
      phi=pi/4.d0
      write (117,'(i4,5f12.6)') ip, phi
      write (118,'(i4,5f12.6)') ip, phi
      ...
      Hint11=3.0d0*uprim
      Hint12=0d0!3.d0*uprim!0d0
      Hint21=0d0!3.d0*uprim!0d0
      Hint22=3.0d0*uprim

      Hon11=0d0!uhubb1/2.d0
      Hon22=0d0!uhubb1/2.d0
...
!      print *,'hello'
      ...
      requirement=0.000000000000001d0
      diff=1000d0

      nbr=0

      do while (diff.gt.requirement)
      ...
      if(nbr.gt.1)then
      Hint12=Hint12_1*cdexp(-xi*qy)
     .      +Hint12_2*cdexp(xi*( dsqrt(3d0)/2d0*qx + (1d0/2d0)*qy ) )
     .      +Hint12_3*cdexp(-xi*( dsqrt(3d0)/2d0*qx - (1d0/2d0)*qy ) )
  
      Hint21 = dconjg(Hint12)
      endif
    
      icc = (iqx-1)*(nkabc(2)) + iqy 
      ...
      rtemp1     = dcos(dsqrt(3.d0)*qx - phi)
      rtemp2     = dcos(-dsqrt(3.d0)*qx/2.d0 + 3.d0*qy/2.d0 - phi)
      rtemp3     = dcos(-dsqrt(3.d0)*qx/2.d0 - 3.d0*qy/2.d0 - phi)

      h1(1,1,1)  =  -2.d0*t2*(rtemp1 + rtemp2 + rtemp3) + eA 
     .           + dreal(Hint11) + dreal(Hon11)
     ...
     .           + dreal(Hint22) + dreal(Hon22)

      rtemp1     = dcos(qy)
      rtemp2     = dcos(-dsqrt(3.d0)*qx/2.d0 - qy/2.d0)
      rtemp3     = dcos( dsqrt(3.d0)*qx/2.d0 - qy/2.d0)

      h1(1,2,1)  =  -t1*(rtemp1 + rtemp2 + rtemp3)
     .           + dreal(Hint12)

      rtemp1     = dsin(qy)
      rtemp2     = dsin(-dsqrt(3.d0)*qx/2.d0 - qy/2.d0)
      rtemp3     = dsin( dsqrt(3.d0)*qx/2.d0 - qy/2.d0)
     ...
     .           + dimag(Hint12)
      ...
      do ib      = 1,kdim  
      ekn(icc,ib)    = e1(ib) 
      ffk(icc,ib)    = 1.d0/(1.d0+dexp( beta*(e1(ib)-rmu0) ))

      dffkdE(icc,ib) = -1.d0/(1.d0+dexp( beta*(e1(ib)-rmu0) ))**2.d0*
     .                 beta*dexp(beta*(e1(ib)-rmu0)) 
 

      do i1      = 1,kdim
      ukn(icc,i1,ib) = dcmplx(z1(i1,ib,1),z1(i1,ib,2))
      enddo
      enddo
...
!      print *,'kalle'
      ...
      Hint11=Hint11 + dconjg(ukn(icc,2,1))*ukn(icc,2,1)
      Hint12_1=Hint12_1 + dconjg(ukn(icc,2,1))*ukn(icc,1,1)*
     .        cdexp(xi*qy)
      Hint12_2=Hint12_2 + dconjg(ukn(icc,2,1))*ukn(icc,1,1)*
     .        cdexp(-xi*( dsqrt(3d0)/2d0*qx + (1d0/2d0)*qy ) )
      Hint12_3=Hint12_3 + dconjg(ukn(icc,2,1))*ukn(icc,1,1)*
     .        cdexp(xi*( dsqrt(3d0)/2d0*qx - (1d0/2d0)*qy ) )
      Hint22=Hint22 + dconjg(ukn(icc,1,1))*ukn(icc,1,1)

      Hon11=0d0!Hon11 + dconjg(ukn(icc,1,1))*ukn(icc,1,1)
      Hon22=0d0!Hon22 + dconjg(ukn(icc,2,1))*ukn(icc,2,1)  

!      print *,dconjg(ukn(icc,2,1))*ukn(icc,1,1)
      enddo
      enddo

!      stop
      if(nbr.eq.1)then
      Hint11=Hint11*3d0*uprim/numk
      Hint12_1=Hint12_1*(-uprim)/numk
      Hint12_2=Hint12_2*(-uprim)/numk
      Hint12_3=Hint12_3*(-uprim)/numk      
      Hint22=Hint22*3d0*uprim/numk

      Hon11=0d0!Hon11*uhubb1/numk
      Hon22=0d0!Hon22*uhubb1/numk
      endif

      if(nbr.gt.1)then
      Hint11=alpha1*Hint11*3d0*uprim/numk+
     .       alpha2*Hint11old
      Hint12_1=alpha1*Hint12_1*(-uprim)/numk+
     .         alpha2*Hint12_1old
      Hint12_2=alpha1*Hint12_2*(-uprim)/numk+
     .         alpha2*Hint12_2old
      Hint12_3=alpha1*Hint12_3*(-uprim)/numk+
     .         alpha2*Hint12_3old
      Hint22=alpha1*Hint22*3d0*uprim/numk+
     .       alpha2*Hint22old

      Hon11=0d0!alpha1*Hon11*uhubb1/numk+
!     .      alpha2*Hon11old
      Hon22=0d0!alpha1*Hon22*uhubb1/numk+
!     .      alpha2*Hon22old
      endif

      diff=dabs(dble(Hint11*dconjg(Hint11)+
     .          Hint12_1*dconjg(Hint12_1)+
     .          Hint12_2*dconjg(Hint12_2)+
     .          Hint12_3*dconjg(Hint12_3)+
     .          Hint22*dconjg(Hint22)+
     .          Hon11*dconjg(Hon11)+
     .          Hon22*dconjg(Hon22)
     .                   - (
     -          Hint11old*dconjg(Hint11old)+
     .          Hint12_1old*dconjg(Hint12_1old)+
     .          Hint12_2old*dconjg(Hint12_2old)+
     .          Hint12_3old*dconjg(Hint12_3old)+
     .          Hint22old*dconjg(Hint22old)+
     .          Hon11old*dconjg(Hon11old)+
     .          Hon22old*dconjg(Hon22old) ) ) )


      Hint11old=Hint11
      Hint12_1old=Hint12_1
      Hint12_2old=Hint12_2
      Hint12_3old=Hint12_3
      Hint22old=Hint22
...
* Gap is always at K-point!
      ...
      Hint12=Hint12_1*cdexp(-xi*ky)
     .      +Hint12_2*cdexp(xi*( dsqrt(3d0)/2d0*kx + (1d0/2d0)*ky ) )
     .      +Hint12_3*cdexp(-xi*( dsqrt(3d0)/2d0*kx - (1d0/2d0)*ky ) )
...
* LARGE LOOP
      ...
      kx  =  ux*dsqrt(3.d0)/2.d0+xshift
      ky  = -ux/2.d0 + uy+yshift
     ...
     .                                            ekn(ikk,2),ffk(ikk,2)
      ...
      if(iUUp.eq.18)then
      write(77,'(4f26.18)') kx,ky,ekn(ikk,1),ekn(ikk,2)
      endif
      endif  

      kx  =  ux*dsqrt(3.d0)/2.d0+xshift+qx
      ky  = -ux/2.d0 + uy+yshift+qy
     ...
     .           +  dimag(Hint12)
      ...
      ikpq = (icc-1)*(nkabc(1))*(nkabc(2)) + ikk
...
!      print *,'hejsan'
      ...
      if(nkk.ne.(nkabc(1))*(nkabc(2))) stop

!      print *,'hej'

      do iw = -nw, nw   !Even (for Pol) 
...
!bn,bnp
      ...
      if(iw.ne.0 .or. 
     .   dabs(-ekpqn(ikpq,bn)+ekn(iq,bnp))
     .   .ne.0.d0)then 

      csum1 = csum1 + (-1.d0)*dconjg(ukpqn(ikpq,it,bn))*
     .                        ukn(iq,it,bnp)*
     .                        dconjg(ukn(iq,itp,bnp))*
     .                        ukpqn(ikpq,itp,bn)*
     .                ( 1.d0)/(xi*wkP(iw)-ekpqn(ikpq,bn)+ekn(iq,bnp))*
     .                ( ffkpq(ikpq,bn) - ffk(iq,bnp) )   
...
!      stop
      ...
      if(it.eq.1.and.itp.eq.1)then
      P11(icc,iw) = csum1*volwgt
      endif
      if(it.eq.1.and.itp.eq.2)then 
      P12(icc,iw) = csum1*volwgt
      endif
      if(it.eq.2.and.itp.eq.1)then 
      P21(icc,iw) = csum1*volwgt
      endif
      if(it.eq.2.and.itp.eq.2)then 
      P22(icc,iw) = csum1*volwgt
      endif
      enddo 
      enddo
...
* eps = (1 - v*P0)
     ...
     o               ceps )
      ...
      eps(1,1)=dcmplx(reps(1,1),ceps(1,1))
      eps(1,2)=dcmplx(reps(1,2),ceps(1,2))
      eps(2,1)=dcmplx(reps(2,1),ceps(2,1))
      eps(2,2)=dcmplx(reps(2,2),ceps(2,2))


      delta=0.0001d0

      deter0=eps(1,1)*eps(2,2)-eps(1,2)*eps(2,1) 

      deter0vals(icc,iw)=deter0


      deter=(deter0*dconjg(deter0)+delta)/dconjg(deter0)

      deter=deter0

      rpi(1,1)=dreal(eps(2,2)/deter)
      rpi(1,2)=dreal(-eps(1,2)/deter)
      rpi(2,1)=dreal(-eps(2,1)/deter)
      rpi(2,2)=dreal(eps(1,1)/deter)

      cpi(1,1)=dimag(eps(2,2)/deter)
      cpi(1,2)=dimag(-eps(1,2)/deter)
      cpi(2,1)=dimag(-eps(2,1)/deter)
      cpi(2,2)=dimag(eps(1,1)/deter)
...
!      write (105,'(i6,4f26.18)') iw,kBZ(1,icc),kBZ(2,icc),
!     .                        Wc(1,1,icc,iw)
...
* Assume real !!
      ...
      if(i1.eq.1) then ! For Wc(1,1,:,:)
      do      iv = -nw, nw
      we(iv)    =  Wc(1,1,ik,iv) 
      enddo
      do      it = 1, ntau
      rsum3=dcmplx(0.d0,0.d0)
      do      iv = -nw, nw  
      rsum3      = rsum3 + wexpwt(iv,it)*we(iv)  
      enddo    
      rsum3=rsum3+
     .      analytic_pos(it)/beta*
     .      wkP(nw)**2.d0*Wc(1,1,ik,nw)+
     .      analytic_neg(it)/beta*
     .      wkP(-nw)**2.d0*Wc(1,1,ik,-nw)
      wktau(1,1,ik,it) = rsum3
      enddo 
      endif

      if(i1.eq.2) then ! For Wc(1,2,:,:)
      do      iv = -nw, nw
      we(iv)    = Wc(1,2,ik,iv)
      enddo
      do      it = 1, ntau
      ...
      rsum3=rsum3+
     .      analytic_pos(it)/beta*
     .      wkP(nw)**2.d0*Wc(1,2,ik,nw)+
     .      analytic_neg(it)/beta*
     .      wkP(-nw)**2.d0*Wc(1,2,ik,-nw)
      wktau(1,2,ik,it) = rsum3
      enddo
      endif

      if(i1.eq.3) then ! For Wc(2,1,:,:)
      do      iv = -nw, nw
      we(iv)    = Wc(2,1,ik,iv)
      enddo
      do      it = 1, ntau
      ...
      rsum3=rsum3+
     .      analytic_pos(it)/beta*
     .      wkP(nw)**2.d0*Wc(2,1,ik,nw)+
     .      analytic_neg(it)/beta*
     .      wkP(-nw)**2.d0*Wc(2,1,ik,-nw)
      wktau(2,1,ik,it) = rsum3
      enddo
      endif

      if(i1.eq.4) then ! For Wc(2,2,:,:)
      do      iv = -nw, nw
      we(iv)    = Wc(2,2,ik,iv)
      enddo
      do      it = 1, ntau
      ...
      rsum3=rsum3+
     .      analytic_pos(it)/beta*
     .      wkP(nw)**2.d0*Wc(2,2,ik,nw)+
     .      analytic_neg(it)/beta*
     .      wkP(-nw)**2.d0*Wc(2,2,ik,-nw)
      wktau(2,2,ik,it) = rsum3
      enddo
      endif
...
* Fix q !
      ...
      do iqy = 1, nkabc(2) 
      uqy=uqy+duy
      iccq = (iqx-1)*(nkabc(2)) + iqy  
      qx   =  uqx*dsqrt(3.d0)/2.d0+xshift
      qy   = -uqx/2.d0 + uqy+yshift
      ...
      do ik =  1, nkk   !Sum k
      ikpq = (iccq-1)*(nkabc(1))*(nkabc(2)) + ik
*      do im  = 1,kdim
      do it  = 1,kdim
      do itp = 1,kdim
...
* im = 1 and im=2 seperately !!
     ...
     .        wktau(it,itp,ik,itt)*
     .       (-ffkpq(ikpq,1))*dexp( ekpqn(ikpq,1)*(beta-tau(itt)) ) !Occ G(tau)
     .
     .              + dconjg(ukpqn(ikpq,itp,2))*ukn(iccq,itp,ibp)*
     .                dconjg(ukn(iccq,it,ib))*ukpqn(ikpq,it,2)*
     .        wktau(it,itp,ik,itt)*
     .       (ffkpq(ikpq,2)-1.d0)*dexp(-ekpqn(ikpq,2)*tau(itt) )    !Unocc G(tau)

! Zero temperature
!      csum1=csum1+dconjg(ukpqn(ikpq,it,1))*ukn(iq,it,2)*
!     .            dconjg(ukn(iq,itp,2))*ukpqn(ikpq,itp,1)*
!     .            1.d0/(wr(iw)-ekn(iq,2)+ekpqn(ikpq,1)+idelta)
!     .           -ukpqn(ikpq,it,1)*dconjg(ukn(iq,it,2))*
!     .            ukn(iq,itp,2)*dconjg(ukpqn(ikpq,itp,1))*
!     .            1.d0/(wr(iw)+ekn(iq,2)-ekpqn(ikpq,1)-idelta)
...
!      stop
* Calculate Off-Diag SigmaEx(1:2,1:2,1:nkk)
!      rw1(1,2) = uprim*dreal( cdexp(-xi*qy) 
!     .         +cdexp(xi*( dsqrt(3d0)/2d0*qx + (1d0/2d0)*qy))
!     .         +cdexp(-xi*( dsqrt(3d0)/2d0*qx - (1d0/2d0)*qy))) 
!      rw1(2,1) = rw1(1,2)
...
!      cw1(2,1) = -cw1(1,2)
      ...
      U12 = uprim*(cdexp(-xi*ky) 
     .         +cdexp(xi*( dsqrt(3d0)/2d0*kx + (1d0/2d0)*ky))
     .         +cdexp(-xi*( dsqrt(3d0)/2d0*kx - (1d0/2d0)*ky))) 
      U21 = dconjg(U12) 
      ...
      ! 11 Component
      csum1=csum1+
     . ffkpq(ikpq,ib)*(U12*dconjg(ukn(icc,1,1))*ukpqn(ikpq,1,ib)*
     .                     dconjg(ukpqn(ikpq,2,ib))*ukn(icc,2,1)+
     .                 U21*dconjg(ukn(icc,2,1))*ukpqn(ikpq,2,ib)*
     .                     dconjg(ukpqn(ikpq,1,ib))*ukn(icc,1,1))


      ! 12 Component
      csum2=csum2+
     . ffkpq(ikpq,ib)*(U12*dconjg(ukn(icc,1,1))*ukpqn(ikpq,1,ib)*
     .                     dconjg(ukpqn(ikpq,2,ib))*ukn(icc,2,2)+
     .                 U21*dconjg(ukn(icc,2,1))*ukpqn(ikpq,2,ib)*
     .                     dconjg(ukpqn(ikpq,1,ib))*ukn(icc,1,2))

      ! 21 Component
      csum3=csum3+
     . ffkpq(ikpq,ib)*(U12*dconjg(ukn(icc,1,2))*ukpqn(ikpq,1,ib)*
     .                     dconjg(ukpqn(ikpq,2,ib))*ukn(icc,2,1)+
     .                 U21*dconjg(ukn(icc,2,2))*ukpqn(ikpq,2,ib)*
     .                     dconjg(ukpqn(ikpq,1,ib))*ukn(icc,1,1))
     ...
     .                     dconjg(ukpqn(ikpq,1,ib))*ukn(icc,1,2))
      ...
      SigmaEx(1,1,icc)=-csum1*volwgt      
      SigmaEx(1,2,icc)=-csum2*volwgt
      SigmaEx(2,1,icc)=-csum3*volwgt
      SigmaEx(2,2,icc)=-csum4*volwgt 
      ...
      ! 11 Component
      csum1=csum1+
     .  ffk(ikk,ib)*(U12*dconjg(ukn(icc,1,1))*ukn(icc,1,1)*
     .                   dconjg(ukn(ikk,2,ib))*ukn(ikk,2,ib)+ 
     .               U21*dconjg(ukn(icc,2,1))*ukn(icc,2,1)*
     .                   dconjg(ukn(ikk,1,ib))*ukn(ikk,1,ib))

      ! 12 Component
      csum2=csum2+
     .  ffk(ikk,ib)*(U12*dconjg(ukn(icc,1,1))*ukn(icc,1,2)*
     .                   dconjg(ukn(ikk,2,ib))*ukn(ikk,2,ib)+ 
     .               U21*dconjg(ukn(icc,2,1))*ukn(icc,2,2)*
     .                   dconjg(ukn(ikk,1,ib))*ukn(ikk,1,ib))

      ! 21 Component
      csum3=csum3+
     .  ffk(ikk,ib)*(U12*dconjg(ukn(icc,1,2))*ukn(icc,1,1)*
     .                   dconjg(ukn(ikk,2,ib))*ukn(ikk,2,ib)+ 
     .               U21*dconjg(ukn(icc,2,2))*ukn(icc,2,1)*
     .                   dconjg(ukn(ikk,1,ib))*ukn(ikk,1,ib))

      ! 22 Component
      csum4=csum4+
     .  ffk(ikk,ib)*(U12*dconjg(ukn(icc,1,2))*ukn(icc,1,2)*
     .                   dconjg(ukn(ikk,2,ib))*ukn(ikk,2,ib)+ 
     .               U21*dconjg(ukn(icc,2,2))*ukn(icc,2,2)*
     .                   dconjg(ukn(ikk,1,ib))*ukn(ikk,1,ib))
...
!      print *, csum1
      ...
      VHartree(1,1,icc)=dreal(csum1)*volwgt
      VHartree(1,2,icc)=dreal(csum2)*volwgt
      VHartree(2,1,icc)=dreal(csum3)*volwgt
      VHartree(2,2,icc)=dreal(csum4)*volwgt
 
    
      enddo

!      VHartree(:,:,:)=0.d0
!      SigmaEx(:,:,:)=dcmplx(0.d0,0.d0)

!      print *,VHartree(1,1,242)
!      print *,SigmaEx(1,2,429)
...
!      stop
      ...
      do iwl = -nw,nw-1
      write(15,'(5f12.6)')wkS(iwl),SigmaC(1,1,iqp,iwl)
      write(16,'(5f12.6)')wkS(iwl),SigmaC(1,2,iqp,iwl)
      write(17,'(5f12.6)')wkS(iwl),SigmaC(2,1,iqp,iwl)
      write(18,'(5f12.6)')wkS(iwl),SigmaC(2,2,iqp,iwl)
      enddo
      endif
...
*      do iq  = 1, iqp    
      ...
      rw3(1,1) = dreal(xi*wkS(iw)-ekn(iq,1)-SigmaC(1,1,iq,iw)
     .           -SigmaEx(1,1,iq)-VHartree(1,1,iq)+delmu)   
      cw3(1,1) = dimag(xi*wkS(iw)-ekn(iq,1)-SigmaC(1,1,iq,iw)
     .           -SigmaEx(1,1,iq)+delmu)

      rw3(1,2) = dreal( -SigmaC(1,2,iq,iw) -SigmaEx(1,2,iq) 
     .                  -VHartree(1,2,iq) )
      cw3(1,2) = dimag( -SigmaC(1,2,iq,iw) -SigmaEx(1,2,iq) )

      rw3(2,1) = dreal( -SigmaC(2,1,iq,iw) -SigmaEx(2,1,iq) 
     .                  -VHartree(2,1,iq) )
      cw3(2,1) = dimag( -SigmaC(2,1,iq,iw) -SigmaEx(2,1,iq) )

      rw3(2,2) = dreal(xi*wkS(iw)-ekn(iq,2)-SigmaC(2,2,iq,iw)
     .           -SigmaEx(2,2,iq)-VHartree(2,2,iq)+delmu)
      cw3(2,2) = dimag(xi*wkS(iw)-ekn(iq,2)-SigmaC(2,2,iq,iw)
     .           -SigmaEx(2,2,iq)+delmu)
...
!      icc =iqp
      ...
      write(19,'(a20,2f12.6)')'q:',kBZ(1,iqp)/twopi,kBZ(2,iqp)/twopi
      write(20,'(a20,2f12.6)')'q:',kBZ(1,iqp)/twopi,kBZ(2,iqp)/twopi
      write(21,'(a20,2f12.6)')'q:',kBZ(1,iqp)/twopi,kBZ(2,iqp)/twopi
      write(22,'(a20,2f12.6)')'q:',kBZ(1,iqp)/twopi,kBZ(2,iqp)/twopi

      do iw = -nw,nw-1
      write(19,'(5f12.6)'),wkS(iw), green11(iqp,iw),
     .1.d0/(xi*wkS(iw)-ekn(iqp,1))
      write(20,'(5f12.6)'),wkS(iw), green12(iqp,iw)
      write(21,'(5f12.6)'),wkS(iw), green21(iqp,iw)
      write(22,'(5f12.6)'),wkS(iw), green22(iqp,iw),
     .1.d0/(xi*wkS(iw)-ekn(iqp,2))
      enddo
      endif
...
!      if(idel.eq.ndel)then
!      write(49,'(i4,3f26.18)') iq,tau(it),rgtau11(it),cgtau11(it)
!      write(50,'(i4,3f26.18)') iq,tau(it),rgtau12(it),cgtau12(it)
!      write(51,'(i4,3f26.18)') iq,tau(it),rgtau21(it),cgtau21(it)
!      write(52,'(i4,3f26.18)') iq,tau(it),rgtau22(it),cgtau22(it)
!      endif
...
*      rtemp2 = rtemp2 + (-rgtau22(1))
      ...
      if(idel.eq.ndel)then
      write (117,'(i4,6f26.18)') iq, kBZ(1,iq),kBZ(2,iq),
     .                         -rgtau11(ntau),  
     .                         -rgtau12(ntau),
     .                         -rgtau21(ntau),
     .                         -rgtau22(ntau)

      write (118,'(i4,6f26.18)') iq, kBZ(1,iq),kBZ(2,iq),
     .                         -cgtau11(ntau),  
     .                         -cgtau12(ntau),
     .                         -cgtau21(ntau),
     .                         -cgtau22(ntau)
...
!      stop
      ...
      ! This tries to fix the chemical potential 
      if(rtemp3/dble(numk).gt.1.d0) delmu = delmu - !stepdelmu
     .                              mumax/2.d0**dble(idel)
...
! Here I should rotate Sigma+VH to atom basis
! and diagonalize for all k as well as for K
! to get Chern number and gap

!      double precision rHXC(2,2,nq0)
!      double precision iHXC(2,2,nq0)

!SigmaC(1,1,iq,iw)-SigmaEx(1,1,iq)-VHartree(1,1,iq)+

      do iq=1,nkk
      HXC(1,1)=(SigmaC(1,1,iq,-1)+SigmaC(1,1,iq,0))/2.d0
     .            +SigmaEx(1,1,iq) + VHartree(1,1,iq)
      HXC(1,2)=(SigmaC(1,2,iq,-1)+SigmaC(1,2,iq,0))/2.d0
     .            +SigmaEx(1,2,iq) + VHartree(1,2,iq)
      HXC(2,1)=(SigmaC(2,1,iq,-1)+SigmaC(2,1,iq,0))/2.d0
     .            +SigmaEx(2,1,iq) + VHartree(2,1,iq)
      HXC(2,2)=(SigmaC(2,2,iq,-1)+SigmaC(2,2,iq,0))/2.d0
     .            +SigmaEx(2,2,iq) + VHartree(2,2,iq)
      ...
      UinvM(1,1)=UM(2,2)/(UM(1,1)*UM(2,2)-UM(1,2)*UM(2,1))
      UinvM(2,2)=UM(1,1)/(UM(1,1)*UM(2,2)-UM(1,2)*UM(2,1))
      UinvM(1,2)=-UM(1,2)/(UM(1,1)*UM(2,2)-UM(1,2)*UM(2,1))
      UinvM(2,1)=-UM(2,1)/(UM(1,1)*UM(2,2)-UM(1,2)*UM(2,1))

      tempmat=matmul(HXC,UinvM)
      Smat=matmul(UM,tempmat)

!      print *, SigmaC(1,1,iq,-1)
     
!      HXCAtom(1,1,iq)=Smat(1,1)
!      HXCAtom(1,2,iq)=Smat(1,2)
!      HXCAtom(2,1,iq)=Smat(2,1)
!      HXCAtom(2,2,iq)=Smat(2,2)

      write (192,'(i4,4f26.18)') iq,dreal(Smat(1,1)),
     .                           dreal(Smat(1,2)),dreal(Smat(2,1)),
     .                           dreal(Smat(2,2))
      write (193,'(i4,4f26.18)') iq,dimag(Smat(1,1)),
     .                           dimag(Smat(1,2)),dimag(Smat(2,1)),
     .                           dimag(Smat(2,2))

!      print *,'VH',VHartree(1,2,iq)
