...
* nphi same as in hgw4.f !!
      ...
      parameter (maxp=1500,kdim=2,maxk=160000) !maxk is new (Just an exaggeratred size for the Green's function for each phi)
     ...
     .                 v2(maxp),v2d(maxp),rgamma(maxp)
      ...
      double precision gammax(2)
      double precision gammay(2)
      double precision uprim,uhubb1
      ...
      double precision mingap,tempgap,avgap,maxgap

      double precision r11,r12,r21,r22
      double precision i11,i12,i21,i22

      double precision rxp11,rxp12,rxp21,rxp22
      double precision ixp11,ixp12,ixp21,ixp22

      double precision ryp11,ryp12,ryp21,ryp22
      double precision iyp11,iyp12,iyp21,iyp22

      integer iUUp,qdum

      complex*16 xpart1(2,2),xpart2(2,2),ypart1(2,2),ypart2(2,2)
...
!      double precision Gnnp(2,2,maxk)  ! Gnnp is new 
      ...
      complex*16 Gnnp(2,2,maxk)

      complex*16 HXC(2,2,maxk)
      complex*16 HXCxp(2,2,maxk)
      complex*16 HXCyp(2,2,maxk)
      complex*16 HXCGamma(2,2)

      complex*16 val1(2,2),val2(2,2)
      ...
      double precision phiC(maxp), tx(2), ty(2), ekn(2),e0t(2)

      complex*16 H0, Hx, Hy, Hz
      complex*16 expphi, ctemp1, ctemp2


!      complex*16 uknbig(nq0,2,2)

      complex*16 ukn(2,2), u0t(2,2), xi, csum, csum1, csum2, csum3
      complex*16 csumk
      complex*16 uknbig(maxk,2,2)
      complex*16 eknbig(maxk,2)
      complex*16 csumx, csumy
...
* Open files.
      ...
      open(unit=292,file='mu.d',status='UNKNOWN')

      open(unit=293,file='uk1.d',status='UNKNOWN')


      open(unit=192,file='rHXC.d',status='UNKNOWN')
      open(unit=193,file='iHXC.d',status='UNKNOWN')
      open(unit=194,file='rHXCpx.d',status='UNKNOWN')
      open(unit=195,file='iHXCpx.d',status='UNKNOWN')
      open(unit=196,file='rHXCpy.d',status='UNKNOWN')
      open(unit=197,file='iHXCpy.d',status='UNKNOWN')

      open(unit=198,file='gap.d',status='UNKNOWN')

      open(unit=199,file='Chern.d',status='UNKNOWN')

      open(unit=116,file='fort.116',status='UNKNOWN') !116 is new (NOT USED)
      open(unit=117,file='ReG.d',status='UNKNOWN') !117 is new
      open(unit=118,file='ImG.d',status='UNKNOWN') !117 is new
...
* For Chern number and IC contribution!
      ...
      do ix = 1, nkabc(1)   !With boundary
*      do ix = 1, nkabc(1)       !No boundary

      ux=ux+dux
      uy=-duy

      do iy = 1, nkabc(2)   !With boundary
...
* Fermifactor.
      ...
      write(293,'(3f26.18)') kx,ky,!dreal(ukn(1,1)*dconjg(ukn(1,1)))
     .                       atan2(dreal(ukn(1,1)+ukn(2,1)),
     .                             dimag(ukn(1,1)+ukn(2,1)) )
...
!      csum=dcmplx(0d0,0d0) ! The third option is that we put csum to 0
                           ! at each ib      
      do ib=1,kdim
      csum=dcmplx(0d0,0d0)
      do i1 = 1,kdim
      csum  = csum+dcmplx(z1  (i1,ib,1),-z1  (i1,ib,2))*
...
!    gammay(2)=1d0
     ...
     .               ukn(i1,ib1)*volapy(ib1,ib2)/(ekn(ib2)-ekn(ib1))
      ...
      ! Here I calculate 1 and 2 and then |dx unk> and |dy unk> 
      ! I have to also calculate deltaE and stuff like that



! Since I have removed x+dx,x-dx,y+dy and y-dy I have to modify the
! derivatives below
...
!!!!! Beginning of new csum1
     ...
     .                    deryEn(ib1)                         !
      ...
      csumk=csumk+Gnnp(ib2,ib1,icount)*ctemp1
                                                        
      enddo
      enddo

!      print *, Gnnp(1,1,icount)
...
!!!!! End of new code
      ...
      if(iUUp.eq.14)then
      write(22,'(3f26.18)') kx,ky,-dimag(csumk*volwgt*fact) ! ne
      endif
...
*      write(6,*)'icount and numk:',icount,numk
      ...
      if(iUUp.eq.0) write(6,'(a30,3f10.2)')'IC contribution to M:'
      if(iUUp.eq.0) write(2,'(a30,3f10.2)')'IC contribution to M:'

      write(6,'(a30,i4,1f26.18)')'M:',iUUp,
     .-dimag(csum1*volwgt*fact) ! new: This change cancels the change in fact

      write(2,'(i4,1f26.18)') iUUp, -dimag(csum1*volwgt*fact) ! new: This
...
! Here I use rHXC and iHXC to calculate gap and Chern number
! I need to interpolate to K point
! Hint components are already calculated
